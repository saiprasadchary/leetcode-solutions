class Solution(object):
    ...
    def dfs(self, row, col, grid, ROWS, COLS, dimensions):
        grid[row][col]=2
        for r, c in dimensions:
            ro=row+r
            co=col+c
            if 0<=ro<ROWS and 0<=co<COLS and grid[ro][co]==1:
                self.dfs(ro, co, grid, ROWS, COLS, dimensions)
    ...
    def numEnclaves(self, grid):
        ...
        for row in range(ROWS):
            for col in range(COLS):
                if(row==0 or row==ROWS-1 or col==0 or col==COLS-1) and grid[row][col]==1:
                    self.dfs(row, col, grid, ROWS, COLS, dimensions)
